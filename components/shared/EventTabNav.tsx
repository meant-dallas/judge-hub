  ...
  return (
    <div className="flex gap-1 border-b border-slate-200 dark:border-slate-700/60">
      {TABS.map((tab) => {
        ...
        const isResultsLocked = tab.key === 'results' && !showResults
          ...
          <Link
            ...
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${
              isActive
                ...
                : isResultsLocked
                ? 'border-transparent text-slate-300 dark:text-slate-600 hover:text-slate-400 dark:hover:text-slate-500 hover:border-slate-200 dark:hover:border-slate-700'
