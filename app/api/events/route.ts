...
export async function POST(req: Request) {
  ...
  const event = await createEvent({ ...parsed.data, created_by: session.user.email!, active_participant_id: '', time_limit_minutes: 0, overtime_deduction: 0, normalize_scores: false })
