      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Link href="/admin/events" className="text-xs text-slate-400 hover:text-slate-600">
              ← Events
            </Link>
          </div>
          <h1 className="text-2xl font-bold text-slate-900">{event.name}</h1>
          <p className="text-sm text-slate-500 mt-1">{event.date}{event.description ? ` · ${event.description}` : ''}</p>
        </div>
      ...
      <section>
        ...
        {participants.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 py-12 text-center">
            <p className="text-slate-400 text-sm">No participants yet. Add the first one above.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wide px-4 py-3">Team</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {participants.map((p) => (
                  <tr key={p.participant_id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-slate-600">{p.team_name || '—'}</td>
                    <td className="px-4 py-3.5 text-slate-600">{p.category || '—'}</td>
                    <td className="px-4 py-3.5 text-slate-600">{p.contact_email || '—'}</td>
      ...
      {criteria.length > 0 && (
        <section>
          <h2 className="text-base font-semibold text-slate-900 mb-3">
            Judging Criteria
            <span className="ml-2 text-xs font-normal text-slate-400">{criteria.length}</span>
          </h2>
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wide px-5 py-3">Criterion</th>
                  <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wide px-4 py-3">Category</th>
                  <th className="text-right text-xs font-semibold text-slate-500 uppercase tracking-wide px-4 py-3">Max Score</th>
                  <th className="text-right text-xs font-semibold text-slate-500 uppercase tracking-wide px-4 py-3">Weight</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {criteria.map((c) => (
                  <tr key={c.criteria_id} className="hover:bg-slate-50">
                    <td className="px-5 py-3.5">
                      <p className="font-medium text-slate-900">{c.name}</p>
                      {c.description && <p className="text-xs text-slate-400 mt-0.5">{c.description}</p>}
                    </td>
                    <td className="px-4 py-3.5 text-slate-600">{c.category || '—'}</td>
                    <td className="px-4 py-3.5 text-right text-slate-700">{c.max_score}</td>
                    <td className="px-4 py-3.5 text-right text-slate-700">{c.weight}×</td>
                  </tr>
                ))}
              </tbody>
            </table>
